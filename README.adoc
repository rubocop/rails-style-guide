...
=== Avoid default_scope [[avoid-default-scope]]

Avoid using `default_scope` in your models. Due to its implicit nature and its affect on associations, its best to go with different solutions as its error prone.

Prefer using explicit scopes where necessary.
...
- https://andycroll.com/ruby/dont-use-default-scope/[Dont use default scope]
- https://www.lewuathe.com/why-we-should-avoid-default_scope-in-rails.html[Why we should avoid default scope]
- https://piechowski.io/post/why-is-default-scope-bad-rails/[Why is default scope bad]
