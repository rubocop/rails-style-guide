...
Prefer `self[:attribute]` over `read_attribute(:attribute)`. The former raises an `ActiveModel::MissingAttributeError` if the attribute is missing, whereas the latter does not.
