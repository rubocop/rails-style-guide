...
在向导中，小节[Testing Rails applications](#testing)是在小节[Developing Rails applications](#developing)之后的，我赞同
[Behaviour-Driven Development](http://en.wikipedia.org/wiki/Behavior_Driven_Development)(BDD)
是开发软件最好的方法。我坚信着。

Rails是一个“固执”的框架，而这也是一个“固执”的向导。我心里非常的肯定
[RSpec](https://www.relishapp.com/rspec)就是比Test::Unit优秀，
[Sass](http://sass-lang.com/)就是比CSS优秀，
而[Haml](http://haml-lang.com/) ([Slim](http://slim-lang.com/))也就是比Erb优秀。
所以，请不要在这跟我提任何有关Test::Unit, CSS 或 Erb的建议。

这里的一些建议只适用于Rails 3.1+.

你可以创建这个向导的一个PDF或HTML的备份，用
[Transmuter](https://github.com/TechnoGate/transmuter)。

# 内容列表

* [Developing Rails applications 开发Rails应用](#developing-rails-applications)
    * [Configuration 配置](#configuration)
    * [Routing 路由](#routing)
    * [Controllers 控制器](#controllers)
    * [Models 模型](#models)
    * [Migrations 数据迁移](#migrations)
    * [Views 视图](#views)
    * [Assets](#assets)
    * [Mailers](#mailers)
    * [Bundler 构建器](#bundler)
    * [Priceless Gems 非常牛的Gems](#priceless-gems)
    * [Flawed Gems 有缺陷的Gems](#flawed-gems)
    * [Managing processes 控制进程](#managing-processes)
    * [Testing Rails applications 测试Rails应用](#testing-rails-applications)
    * [Cucumber](#cucumber)
    * [RSpec](#rspec)
...
* 把自定义的启动代码（initialization code）放在`config/initializers`里。在initializers
   文件夹里的代码将会在应用启动时被执行。
* 每一个gem的启动代码（initialization code）必须放在分开的文件里，并以和gem相同
   的名字命名，比如`carrierwave.rb`，`active_admin.rb`，等等。
* 为开发环境，测试环境和生产环境配置相应的内容。（在`config/environments/`文件夹
   下对应的文件中）
  * 为预编译额外的静态内容做标记（如果有的话）：
...
* 创建一个额外的阶段环境（`staging` environment），非常接近于生产环境（`production` environment）。

## Routing
## 路由

* 当你需要为你的应用增加更多指向RESTful资源的方法(你真的需要他们吗？)，
使用 `member` 和 `collection` 。
